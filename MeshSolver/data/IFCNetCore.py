...
import pymeshlab
from MeshSolver.data.preprocess import find_neighbor
...
class IFCNetCore(data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        if num_point < self.max_faces:
            #print('Warning: number of faces is less than max')
            fill_face = []
            fill_neighbor_index = []
            ...
            #print(len(face), '-> new len of faces')
            neighbor_index = np.concatenate((neighbor_index, np.array(fill_neighbor_index)))

        # ### JUST A WORKAROUND TO SEE IF THE REST WORKS FOR NOW ###
        # if num_point > self.max_faces:
        #     face = face[:self.max_faces]
        #     neighbor_index = neighbor_index[:self.max_faces]
